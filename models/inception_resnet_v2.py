...
class Inception_ResNetv2(nn.Module):
    def __init__(self, in_channels=3, classes=1000, k=256, l=256, m=384, n=384):
        ...
        self.block_0 = Stem(in_channels)
        blocks = []
        # blocks.append(Stem(in_channels))  # /2, /2, /2 =[35, 35]
        for i in range(10):
            blocks.append(Inception_ResNet_A(320, 0.17))
        blocks.append(Reduction_A(320, k, l, m, n))  # /2 [17, 17]
        self.block_1 = nn.Sequential(*blocks)
        blocks = []
        for i in range(20):
            blocks.append(Inception_ResNet_B(1088, 0.10))
        blocks.append(Reduciton_B(1088))  # /2  [8, 8]
        self.block_2 = nn.Sequential(*blocks)
        blocks = []
        for i in range(9):
            blocks.append(Inception_ResNet_C(2080, 0.20))
        blocks.append(Inception_ResNet_C(2080, activation=False))
        self.block_3 = nn.Sequential(*blocks)
    ...
    def forward(self, x):
        conv_features = []
        # x = self.features(x)
        x = self.block_0(x)  # [35, 35]
        conv_features.append(x)
        x = self.block_1(x)  # [17, 17]
        conv_features.append(x)
        x = self.block_2(x)  # [8, 8]
        conv_features.append(x)
        x = self.block_3(x)  # [8, 8]
        x = self.conv(x)
...
if __name__ == "__main__":
    ...
    model = Inception_ResNetv2(classes=2)
